...
class ModManager(QWidget):
    def __init__(self):
        super().__init__()
        # --- MODIFIED ---: Determine the application's base directory (portable mode)
        if getattr(sys, 'frozen', False):
            # If running as a bundled exe, the base path is the exe's directory
        ...
        # --- MODIFIED ---: All paths are now relative to the application's base directory
        self.settings_file = os.path.join(self.app_base_dir, "mods_folder_settings.json")
        self.dat_settings_file = os.path.join(self.app_base_dir, "dat_settings.json")
        self.backup_dir = os.path.join(self.app_base_dir, "backup")
    ...
    def set_windows11_dark_theme(self):
        ...
        app.setStyleSheet("""
            ...
            QScrollBar::handle:vertical { background: #0078d7; min-height: 30px; border-radius: 6px; }
            QScrollBar::handle:vertical:hover, QScrollBar::handle:vertical:pressed { background: #0078d7; }
            QScrollBar::add-line:vertical, QScrollBar::sub-line:vertical { border: none; background: none; }
            QScrollBar::add-page:vertical, QScrollBar::sub-page:vertical { background: none; }
            QScrollBar:horizontal { border: none; background: #202020; height: 12px; margin: 0px; }
            QScrollBar::handle:horizontal { background: #0078d7; min-width: 30px; border-radius: 6px; }
            QScrollBar::handle:horizontal:hover, QScrollBar::handle:horizontal:pressed { background: #0078d7; }
    ...
    def autodetect_mods_folder(self):
        """Checks for a 'mods' folder in the script's directory and sets it as default if empty."""
        # --- MODIFIED ---: This logic now correctly defaults to a "mods" folder next to the .exe
        if not self.settings.get("mods_folder"):
            local_mods_path = os.path.join(self.app_base_dir, 'mods')
    ...
    # --- Core Mod Logic ---
    # (The rest of the functions below this point do not need any changes)
